...
ON
	irp.item_record_id = bril.item_record_id
JOIN
	sierra_view.checkout as c
ON
	c.item_record_id = irp.item_record_id	
JOIN	
	sierra_view.patron_record_fullname as prf
ON
	prf.patron_record_id = c.patron_record_id
JOIN	
	sierra_view.patron_view as pv
ON
	pv.id = c.patron_record_id
JOIN
	sierra_view.hold as h
ON
	h.patron_record_id = c.patron_record_id	
JOIN	
...
AND
	brp.best_title like '%'	
GROUP BY
	brl.location_code, brp.best_title, irp.barcode, prf.first_name, prf.last_name, pv.barcode
ORDER BY
	brp.best_title