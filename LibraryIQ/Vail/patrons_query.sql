-- Patrons Query for LibraryIQ
-- Retrieves patron record data for LibraryIQ reporting
SELECT
	...
	p.home_library_code AS "PatronBranch",
	...
	TO_CHAR(
		(
			SELECT
				MAX(rh.checkout_gmt)
			FROM
				sierra_view.reading_history rh
			WHERE
				rh.patron_record_metadata_id = rmp.id
		),
...
FROM
	...
	JOIN sierra_view.ptype_property_myuser pt ON p.ptype_code = pt.value
	JOIN sierra_view.record_metadata rmp ON p.id = rmp.id
	JOIN sierra_view.patron_record_address a ON p.id = a.patron_record_id
	AND a.patron_record_address_type_id = 1
	--Pull full file on Fridays, delta file other days
WHERE
	p.patron_agency_code_num = 88
	AND CASE
		WHEN EXTRACT(
			DOW
			FROM
				CURRENT_DATE
		) = 5 THEN rmp.record_last_updated_gmt::DATE <= CURRENT_DATE
