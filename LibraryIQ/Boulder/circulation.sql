-- Subquery 1: Patrons from agency 3 or 8
SELECT
...
FROM
    ...
    -- For renewals, find the most recent prior checkout for same item+patron
    LEFT JOIN LATERAL (
        SELECT c.stat_group_code_num
        FROM sierra_view.circ_trans c
        WHERE c.item_record_id   = t.item_record_id
          AND c.patron_record_id = t.patron_record_id
          AND c.transaction_gmt  < t.transaction_gmt
          AND c.op_code          = 'o'
        ORDER BY c.transaction_gmt DESC
        LIMIT 1
    ) co ON t.op_code = 'r'

    -- Use the checkout's stat group for renewals, otherwise the transaction's own
    JOIN sierra_view.statistic_group_myuser sg
      ON sg.code = COALESCE(co.stat_group_code_num, t.stat_group_code_num)

WHERE
    t.op_code IN ('o', 'r', 'u')
    AND t.transaction_gmt::DATE > CURRENT_DATE - INTERVAL '4 days'
    AND p.patron_agency_code_num IN (3)

UNION ALL

-- Subquery 2: Patrons from any agency, items from agency 3
SELECT
    rmi.record_type_code || rmi.record_num AS "ItemNum",
    ip.barcode AS "Barcode",
    rmb.record_type_code || rmb.record_num AS "BibNum",
...
FROM
    ...
    -- Same lateral join for renewals
    LEFT JOIN LATERAL (
        SELECT c.stat_group_code_num
        FROM sierra_view.circ_trans c
        WHERE c.item_record_id   = t.item_record_id
